...
import { SUPERADMIN_JWT_SECRET, SUPERADMIN_SALT } from "../service/jwtSecret";
...
const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Hash password
    const hashedPassword = await bcrypt.hash(password, SUPERADMIN_SALT);
...
const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = generateToken(SuperAdmin._id, SUPERADMIN_JWT_SECRET);
